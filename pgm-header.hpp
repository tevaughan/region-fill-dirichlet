...
inline pgm_header::pgm_header(uint16_t c, uint16_t r, uint16_t m)
    : num_cols_(c), num_rows_(r), max_val_(m) {}

inline std::istream &pgm_header::read(std::istream &is) {
  std::string m;
  is >> m;
  if (m != "P5") {
...
inline std::ostream &pgm_header::write(std::ostream &os) {
