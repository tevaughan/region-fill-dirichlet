/// \file       include/dirichlet/FillExperiment.hpp
/// \copyright  2022 Thomas E. Vaughan.  See terms in LICENSE.
/// \brief      Definition of dirichlet::FillExperiment.

#ifndef DIRICHLET_FILL_EXPERIMENT_HPP
#define DIRICHLET_FILL_EXPERIMENT_HPP
...
///
/// This is experimental code that attempts speed-up by approximating standard
/// solution by one that has square blocks with corners participating in
/// solution to linear problem, but each block's interior is bilinearly
/// interpolated.
///
...
class FillExperiment {
  ...
  FillExperiment(FillExperiment const &)= delete;

  /// Disallow assignment because we store owned pointer.
  FillExperiment &operator=(FillExperiment const &)= delete;
  ...
  static ArrayX2i
  coords(Comp *mask, unsigned width, unsigned height, int stride);
...
void FillExperiment::initMatrix() {
...
void FillExperiment::initCoords(ArrayX2i const &coords) {
...
FillExperiment::FillExperiment(
      ArrayX2i const &coords, unsigned width, unsigned height, bool cg):
...
template<typename Comp>
ArrayX2i FillExperiment::coords(Comp *m, unsigned w, unsigned h, int stride) {
  Comp const zero(0);
...
/// Find smallest power of two equal to or greater than `s`.
/// \param s  Lower bound of return-value.
/// \return   Smallest power of two equal to or greater than `s`.
inline int upperBoundPow2(int s) {
  int p= 0;
  int r= (1 << p);
  while(r < s && p < 30) {
    ++p;
  ...
  if(p > 30) throw "maximum power exceeded";
  return r;
}
...
template<typename Comp>
FillExperiment::FillExperiment(
      Comp const *mask, unsigned w, unsigned h, int s, bool cg):
    FillExperiment(coords(mask, w, h, s), w, h, cg) {
  // Type used for mask-images.
  using Image= Array<Comp, Dynamic, Dynamic, RowMajor>;
  Eigen::Map<Image const> m(mask, h, w); // Image-view of mask.
  vector<Image>           v; // Vector of masks, each with different binning.
  // Extend unbinned mask to power of two along each dimension.
  int const w0= upperBoundPow2(w);
  int const h0= upperBoundPow2(h);
  // Make first mask in vector of binned masks be the unbinned one.
  v.push_back(Image::Zero(h0, w0));      // Initialize with zeros.
  v[0](seq(0, h - 1), seq(0, w - 1))= m; // Copy data from input.
  // Find maximum binning with fully masked binned square.
  bool done= false;
  int  log2= 1; // Logarithm (base-2) of single-dimension binning-factor.
  while(!done) {
    int const n = (1 << log2); // Single-dimension binning-factor.
    int const wn= w0 / n;
    int const hn= h0 / n;
    v.push_back(Image(hn, wn));
    Image       &cur= v[v.size() - 1];
    Image const &prv= v[v.size() - 2];
    auto const p00= prv(seq(0, prv.rows() - 2, 2), seq(0, prv.cols() - 2, 2));
    auto const p10= prv(seq(1, prv.rows() - 1, 2), seq(0, prv.cols() - 2, 2));
    auto const p01= prv(seq(0, prv.rows() - 2, 2), seq(1, prv.cols() - 1, 2));
    auto const p11= prv(seq(1, prv.rows() - 1, 2), seq(1, prv.cols() - 1, 2));
    cur           = p00 + p10 + p01 + p11;
    int const  n2 = n * n;
    auto const fc= (cur(seq(1, cur.rows() - 2), seq(1, cur.cols() - 2)) == n2);
    auto const fb= (cur(seq(2, cur.rows() - 1), seq(1, cur.cols() - 2)) == n2);
    auto const ft= (cur(seq(0, cur.rows() - 3), seq(1, cur.cols() - 2)) == n2);
    auto const fl= (cur(seq(1, cur.rows() - 2), seq(0, cur.cols() - 3)) == n2);
    auto const fr= (cur(seq(1, cur.rows() - 2), seq(2, cur.cols() - 1)) == n2);
    auto const f = fc && fb && ft && fl && fr;
    int const  s = f.template cast<int>().sum();
    if(s == 0) {
      done= true;
    } else {
      ++log2;
...
template<typename Map> VectorXf FillExperiment::solve(Map const &im) const {
...
template<typename Map>
void FillExperiment::copySolutionBackIntoImage(
      Map &im, VectorXf const &x) const {
  using Comp= typename Map::CompType;
  // Image is row-major.
  auto const ii= /*col*/ coords_.col(0) * wdth_ + /*row*/ coords_.col(1);
  if constexpr(is_integral_v<Comp>) {
...
template<typename Comp>
VectorXf FillExperiment::operator()(Comp *image, int stride) const {
...
#endif // ndef DIRICHLET_FILL_EXPERIMENT_HPP
